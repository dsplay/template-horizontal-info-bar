...
### News widget depends on a third-party CORS proxy (`api.allorigins.win`) that is not reliable

`src/components/news/index.jsx` fetches the RSS feed through `https://api.allorigins.win/get?url=...` because `rss_url` is typically a cross-origin URL and `rss-parser` can't read it directly from the browser. This free proxy is known to be flaky — as of this writing it's returning Cloudflare `520`/`522` errors for *any* URL, confirmed independently via `curl`, unrelated to anything in this repo. The component already fails gracefully (an empty `<div className="block news" />`, no crash) when the fetch fails, so this reads as "the widget shows nothing" rather than an error. If this proxy's unreliability becomes a recurring problem, consider swapping it for a different CORS proxy or a small server-side fetch endpoint — not done here since it's a third-party outage, not a code bug.
